module ApplicationHelper
  ...
  def reading_level_help(locale,level)
    I18n.locale = locale
    default_text = t("story.cover_text.reading_levels.level_#{level}", locale: :en)
    text = t("story.cover_text.reading_levels.level_#{level}", default: default_text)
    I18n.locale = :en
    text
  end
  ...
  def illustration_crop_style_pdf(page)
    left, top, width, height = get_image_dimensions(page)
    page.image_type == "image/gif" ? "position: relative; top: #{-top}%; left:#{-left}%; height: #{height}%; width: #{width}%;" : ""
  end

  def illustration_crop_style(page)
    left, top, width, height = get_image_dimensions(page)
    unit = page.image_type == "image/gif" ? "%" : "px"
    "position: relative; top: #{-top}#{unit}; left:#{-left}#{unit}; height: #{height}#{unit}; width: #{width}#{unit};"
  end

  def preview_illustration_crop_style(page)
    crop_ratio = 1.038
    left, top, width, height, unit = get_image_dimensions_unit(page, crop_ratio)
    "position: relative; top: #{-top}#{unit}; left:#{-left}#{unit}; height: #{height}#{unit}; width: #{width}#{unit};"
   end
  ...
  def get_cover_image_crop_coords(story)
    story.get_cover_image_crop_coords
  end
  ...
  def get_download_link(story, type)
    pdf_download_version_story = story
    if story.download_version.present?
      download_story = Story.find_by_id story.download_version
      pdf_download_version_story = download_story if download_story.present?
    end

    if(type=="ePub")
      story.published? ? download_story_url(story, format: :epub) : story_page_url(story, story.pages.first, format: :epub) 
    else
      is_high_resolution = type == "HiRes PDF" ? true : nil
      pdf_download_version_story.published? ? download_story_url(pdf_download_version_story, format: :pdf, high_resolution: is_high_resolution) : story_page_url(pdf_download_version_story, pdf_download_version_story.pages.first, format: :pdf , high_resolution: is_high_resolution)
    end
  end
...
def default_date_format(date)
  return date.localtime.strftime("%d %b %y at %H:%M")
end
...
def escape_characters_for_xml(str)
  escape_special_characters(str).sub("&", "&amp;").sub("<", "&lt;").sub(">", "&gt;").sub("\"", "&quot;").sub("'", "&apos;")
end

def ist(time)
  time.in_time_zone(TZInfo::Timezone.get('Asia/Kolkata')).to_s.chomp(" +0530")
end