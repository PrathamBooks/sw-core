...
json.data do |json|
  ...
  json.id @list.id
  json.datePublished @list.created_at.localtime.strftime("%d-%m-%Y")
  json.dateModified @list.updated_at.localtime.strftime("%d-%m-%Y")
  json.categories @list.categories.pluck(:name)
  json.count @list.stories.count
  json.liked current_user ? @list.likes.include?(current_user) : false
  json.likeCount @list.likes.count
