...
#  story_download_count        :integer          default(0)
#  illustration_download_count :integer          default(0)
...
class User < ActiveRecord::Base
  ...
  searchkick match: :word_start,
             searchable: [:email, :name, :first_name, :last_name],
             merge_mappings: true
  ...
  ANONYMOUS_USER_EMAIL =  "anonymous@sample.com"

  has_attached_file :profile_image,
  default_url: ->(attachment) { ActionController::Base.helpers.asset_path('profile_image.jpg') },
  ...
  enum role: {user: 0, reviewer: 1, content_manager: 2, publisher: 3, admin: 4, recommender: 5, promotion_manager: 6, translator: 7, partnerships_manager: 8}
  ...
  has_many :smiley_stories, through: :smiley_ratings, source: :story
  has_one :user_token
  has_many :translation_feedbacks
  has_many :translation_stories, through: :translation_feedbacks, source: :story
  ...
  scope :partnerships_manager, -> { where("site_roles LIKE ?", "%partnerships_manager%") }
  ...
  after_update :reindexing

  def make_user_anonymous
    unless roles.any? || organization.present?
      anonymous_user = User.find_by(email: ANONYMOUS_USER_EMAIL)
      anonymous_user ||= User.create(first_name: "anonymous", email: ANONYMOUS_USER_EMAIL, password: "12345678", password_confirmation: "12345678")
      anonymous_user.phonestories_popup = self.phonestories_popup
      self.phonestories_popup.try(:delete)
      ["stories", "story_downloads", "story_reads", "re_published_stories", "illustration_downloads", "lists", "list_likes", "list_views", "list_downloads", "ratings", "smiley_ratings", "reviewer_comments", "reviewer_stories", "media_mentions", "albums", "user_popups"].each do |item|
        anonymous_user.send(item) << self.send(item)
        if item == "stories"
          anonymous_user.stories.where(:copy_right_holder_id => self.id).update_all(:copy_right_holder_id => anonymous_user.id)
          anonymous_user.stories.reindex
        elsif item == "lists"
          anonymous_user.lists.reindex
        end
        self.send(item).delete_all
      end
      if self.person.present?
        person = anonymous_user.person
        person ||= Person.create!(first_name: anonymous_user.first_name, user_id: anonymous_user.id)
        anonymous_user.person.illustrations << self.person.illustrations
        anonymous_user.person.illustrations.where(:uploader_id => self.id).update_all(:uploader_id => anonymous_user.id)
        anonymous_user.person.illustrations.where(:copy_right_holder_id => self.id).update_all(:copy_right_holder_id => anonymous_user.id)
        anonymous_user.person.illustrations.reindex
        self.person.try(:delete)
      end
      anonymous_user.save
      self.destroy
    else
      puts "You can only delete normal user."
    end
  end
  
  # override devise method to ban users
  # Ref: https://stackoverflow.com/questions/6004216/devise-how-do-i-forbid-certain-users-from-signing-in
  def active_for_authentication?
    super and not self.is_banned?
  end

  # show custom message when a user is banned.
  def inactive_message
    is_banned? ? "Sorry, this account has been deactivated." : super
  end
  ...
  def search_data()
    {
        ...
        has_own_stories: has_own_stories
    }
  end
  ...
  def is_language_reviewer(language_name)
    return true if self.content_manager?
  ...
  def roles
    ...
    rs << "partnerships_manager" if self.partnerships_manager?
    rs
  end
  ...
  def partnerships_manager?
    if self.site_roles != nil && self.site_roles.include?("partnerships_manager")
      return true
    else
      return false
    end
  ...
  def has_smiley_rating(story)
    return self.smiley_ratings.where(:story_id => story.id).present?
  end

  def has_story_reviewed(story)
    if story.published? && story.organization.nil? && !(story.authors.include?(self)) && !(story.reviewer_comment.present?)
      return true
    end
    return false
  end
  # Same author for story and illustrations
  def has_own_stories
    hi_res_access = false
    self.stories.each do|story|
      if story.illustrators.collect(&:email).join(",") == story.authors.collect(&:email).join(",")
        hi_res_access = true
      end
    end
    return hi_res_access
  end

  def own_stories
    has_own_stories = User.search(where: {:id => self.id}, load: false).results.try(:first).has_own_stories
  end

  def illustration_download_limit_reached?
    if illustration_download_count < 5
      return false
    elsif illustration_download_count >= 5 && !first_illustration_download_limit
      return true
    else
      return false
    end
  ...
  def reindexing
    if first_name_changed? || last_name_changed?
      stories.each(&:reindex)
      person && person.illustrations.each(&:reindex)
      lists.each(&:reindex)
    end
  end
end
