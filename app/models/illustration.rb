...
class Illustration < ActiveRecord::Base
  ...
  searchkick callbacks: :async,
             ...
             filterable: [:styles, :illustrators, :categories, :organization, :illustrator_slugs, :tags_name, :illustration_downloads, :illustrator_images, :image_content_type],
             mappings: {
               illustration: {
                 properties: {
		   likes: {type: "long"},
  ...
  has_and_belongs_to_many :illustration_downloads, join_table: 'illustrations_downloads'
  ...
  SIZE_BREAKPOINTS = [359, 479, 490, 767, 900, 1365]
   ...
   has_attached_file :image,
    ...
    paperclip_options = {format: attachment.instance.image_format}
    {
      large: paperclip_options.merge(geometry: "1073"),
      search: paperclip_options.merge(geometry: "260"),
      original_for_web: paperclip_options.merge(geometry: "1000x1000<")
    }
  },
  ...
  def image_content_type_valid
   if !uploader.try(:allow_gif_images) && !['image/jpeg', 'image/jpg', 'image/png'].include?(image.content_type)
     errors[:base] <<  'PNG and JPEG are the only supported formats'
   elsif (uploader.try(:allow_gif_images) && !['image/jpeg', 'image/jpg', 'image/png', 'image/gif' ].include?(image.content_type))
     errors[:base] <<  'GIF, PNG and JPEG are the only supported formats'
   end
  end
  ...
  def generate_smart_crop_details
    ...
    output = `nodejs "#{smart_crop_js}" "#{load_image_path}" 224 224`
  ...
  def search_data()
    {
      ...
      illustrator_emails: illustrators.map(&:email),
      ...
      illustration_downloads: illustration_downloads.map(&:user_id),
      illustrator_images: illustrators.map(&:email)
    }
  end
  ...
  def generate_crop_details(crop_details)
    width = self.image_geometry.width
    height = self.image_geometry.height
    if crop_details['y'] == 0 && crop_details['x'] == 0
      crop_x = crop_y = width > height ? height : width
    elsif crop_details['y'] == 0
      crop_x = crop_y = height	
    else
      crop_x = crop_y = width
    end
    return {
      'x' => crop_details['x'] * width,
      'y' =>  crop_details['y'] * height,
      'ratio_x' => 1,
      'ratio_y' => 1,
      'crop_x' => crop_x,
      'crop_y' => crop_y
  ...
  def process_illustration_smart_crop(page, illustration_crop)
    story_card_crop = illustration_crops.new
    crop_details = generate_crop_details(generate_smart_crop_details)
    story_card_crop.save_crop_data!(crop_details)

    story = page.story
    story.create_story_card(story_card_crop, page)
  end

  def process_crop!(page, local_image = nil, x = 0, y = 0, w = nil, h = nil)
    filename = File.join(Rails.root,'tmp',"#{SecureRandom.hex}")
    ...
    if (img.format == "GIF")
      filename += ".gif"
      imgs = Magick::ImageList.new(load_image_path(local_image))
      imgs.write(filename)
    else
      filename += ".png"
  ...
  def process_crop_background!(illustration_crop, page, local_image = nil, x = 0, y = 0, w = nil, h = nil)
    ...
    page.story.reindex
    page.story.descendants.reindex
    f.close
    File.delete(filename)
  end
  ...
  def number_of_parent_stories_used_in
    illustration_crops.map{|illustration_crop| illustration_crop.pages.map{|p| p.story if p.story.try(:status) == "published" && (p.story.present? && p.story.root?)}}.flatten.compact.uniq.count
  end
  ...
  def parent_story_links
    illustration_crops.map{|illustration_crop| illustration_crop.pages.map{|p| p.story if p.story.try(:status) == "published" && (p.story.present? && p.story.root?)}}.flatten.compact.uniq
  end

  def all_story_links
    illustration_crops.map{|illustration_crop| illustration_crop.pages.map{|p| p.story if p.story.try(:status) == "published" && p.story.present?}}.flatten.compact.uniq
  end
 ...
 def set_contest_illustrators(current_user)
  ...
  def used_in_published_stories
    illustration_crops.map{|illustration_crop| illustration_crop.pages.collect(&:story)}.flatten.compact.map {|story| story if story.published? }.compact.uniq
  end
  ...
  def update_downloads(current_user, ip_address, type)
    ...
    download.organization_user = false
    download.save!
    download.illustrations << self
  end
  ...
  def load_image_path(local_image = nil)
    if !local_image.nil?
      img = local_image
    elsif Rails.env == 'production'
      if self.storage_location.nil?
        img ="public/#{Settings.fog.local_directory}/#{image.path}"
      else
	img = image.public_url
      end
    elsif Rails.env == 'development'
      img = "public/system/story-weaver/#{image.path}"
    else
      img = image.path
    end
  end
  ...
  def load_image(local_image)
    Magick::Image.read(load_image_path(local_image))[0]
  end
