...
class Page < ActiveRecord::Base
  ...
  has_many :textboxes, -> { order(id: :asc) },  as: :page, dependent: :destroy
  ...
  def page_content_update(content,textbox_id)
    if textbox_id.present?
      textboxes.find_by_id(textbox_id).update(content: content)
    else
  ...
  def image_type
    illustration_crop.illustration.image_content_type
  end
