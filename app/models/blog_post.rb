...
#  add_to_home                  :boolean          default(FALSE)
#  image_caption                :text
#
# Indexes
#
...
class BlogPost < ActiveRecord::Base
  ...
  validates_presence_of :body, unless: :draft?
  do_not_validate_attachment_file_type :blog_post_image
  validate :image_content_type_valid, if: :blog_post_image_present?
  validates_presence_of :blog_post_image, unless: :draft?
  has_many :comments
  belongs_to :user
  ...
  def image_content_type_valid
    if !draft? && !['image/jpeg', 'image/jpg', 'image/png'].include?(blog_post_image.content_type)
     errors[:base] <<  'jpeg, jpg and png are only supported formats'
    end
  end

  def blog_post_image_present?
    blog_post_image.present?
  end
end
