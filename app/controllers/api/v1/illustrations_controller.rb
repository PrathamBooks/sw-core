class Api::V1::IllustrationsController < Api::V1::ApplicationController
  ...
  def create
    if params[:image].content_type == "image/gif" && !current_user.try(:allow_gif_images)
      render json: {"ok" => false, "data" => { "errorCode" => 422, "errorMessage" => ["PNG and JPEG are the only supported formats"] }}
    else
      illus_params = illustration_params
      illus_params[:uploader] = current_user
      ...
      if current_user && @illustration.set_illustrator(illus_params, current_user)
        ...
        messages = @illustration.errors.full_messages.uniq.map do |message|
          if message =~ /processing the thumbnail/
            message = "Invalid Image"
          end
          message
        end
        render json: {"ok"=> false, "data" => {"errorCode" => 422, "errorMessage" => messages}}, status: 422
      end
    end
  end
  ...
  def filters
    styles = IllustrationStyle.all.map{|is| {:name => is.translated_name, :queryValue => is.name, id: is.id}}
    categories = IllustrationCategory.all.map{|ic| {:name => ic.translated_name, :queryValue => ic.name, id: ic.id}}
    publishers = Organization.where(organization_type: "Publisher").joins(:illustrations).uniq.order(:organization_name).map{|o| {:name => o.translated_name, :queryValue => o.organization_name}}
    style_filter = {:name => "Style", :queryKey => "style", :queryValues => styles}
    category_filter = {:name => "Category", :queryKey => "category", :queryValues => categories}
    publisher_filter = {:name => "Publisher", :queryKey => "publisher", :queryValues => publishers}
    results = {:style => style_filter, :category => category_filter, :publisher => publisher_filter}
    ...
    download_filter = ["JPEG", "HiRes JPEG"].map{|type| {:name => I18n.t("download_link."+type), :queryValue => type} if current_user && download_type(type) }.compact
  ...
  def download_type(type)
    if type == "JPEG"
      ...
      if current_user.content_manager? || current_user.organization? || (current_user.person && current_user.person.illustrations.any?)
        return true
      end
    end
  end
