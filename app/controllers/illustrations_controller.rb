class IllustrationsController < ApplicationController
  ...
  def index
    ...
    if (params[:editor_fav_images]== "false" && params[:search] && params[:search][:query] == "")
      ...
      unless (current_user.try(:allow_gif_images))
        params.merge!(:no_gif_images => true)
      end
  ...
  def reset_illustration_download_count
    ...
    current_user.first_illustration_download_limit = true unless current_user.first_illustration_download_limit
    current_user.save!
    render json: {"ok"=>true}
  end
  ...
  def download_file(illustration,style)
    if Rails.env.production?
      ...
      extname = File.extname(illustration.image.url(style)).split("?").first
