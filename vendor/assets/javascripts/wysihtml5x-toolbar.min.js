  ...
  wysihtml.commands.formatBlock = {
    exec: function(composer, command, options) {
      ...
      if (ranges.length == 1){
        ...
        while (textContainer.tagName !== 'SPAN' && textContainer.tagName !== 'P'){
...
(function(wysihtml) {
  ...
  wysihtml.commands.formatInline = {
    ...
    cleanEditor: function(composer, outerClass) {    
      ...
      for (var j = 0; j < getPElements.length; j++) {
        ...
        for (var k = 0; k < spanNodes.length; k++) {
          ...
          if (onlySpanNodes(spanNodes[k])) {
            // delete the node and move its children .
      ...
      getAllSpanNodes = composer.element.getElementsByTagName("span")
...
(function(wysihtml) {
  ...
  var handleKeyUp = function(event) {
    ...
    // Issue: SW-1257
    // Cleaning up the editor after pressing delete key or backspace key.
    // Need modification for chromium. In chromium, when we go back to previous line,
    // the style doesn't get saved, instead we get an inlin style with font-size.
...
(function(wysihtml) {
  ...
  wysihtml.Editor = wysihtml.lang.Dispatcher.extend({
    ...
    _cleanAndPaste: function (oldHtml) {
      ...
      beginSel = this.composer.selection.getSelection();
      ...
      dNode = this.composer.doc.getElementById('dummyNode');
