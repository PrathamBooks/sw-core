module Search
  class Illustrations
    ...
    def initialize params
      ...
      @current_user = params[:search][:current_user] if params[:search] && params[:search][:current_user].present?
      ...
      image_content_type = (params[:allow_gif_images] == 'false' || params[:no_gif_images]) ? {not: 'image/gif'} : {}
      search_filters[:image_content_type] = image_content_type
    ...
    def search_query(page, per_page, random_score, search_filters, order_citeria, offset = nil)
      ...
      search_filters[:image_content_type] =  {:not => 'image/gif'}  if @params[:image_content_type].present?
    ...
    def get_filters
      ...
      filters[:illustration_downloads] = ({not: @current_user.id} if @current_user) if @params[:bulk_download].present? #Condition for stories which current user not downloaded
      #filters[:illustrator_images] = @current_user.email  if (@current_user && @current_user.person && @current_user.person.illustrations.any?) && @params[:download_format] == "HiRes JPEG"
    ...
    def search_params(params)
      if params.blank? || params[:search].blank?
        ...
        search_params[:bulk_download] = params[:search][:bulk_download]
        search_params[:download_format] = params[:search][:download_format]
	...
	search_params[:allow_gif_images] = params[:allow_gif_images]
        search_params[:tags] = params[:search][:tags]
	search_params[:image_content_type] = params[:search][:image_content_type]
        search_params
      end
    end
  end
