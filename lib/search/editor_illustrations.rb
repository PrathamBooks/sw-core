module Search
  ...
  class EditorIllustrations
    ...
    def initialize params
      ...
      allow_gif_images =  params[:allow_gif_images] == "true" && User.find_by_id(params['current_user']).try(:allow_gif_images)
      unfilter_image_type = ''
      if !allow_gif_images 
        search_filters[:image_content_type] = {not: "image/gif"}
        unfilter_image_type = 'image/gif'
      end
      ...
      if(@story.pages_with_illustrations.length == 0)
        ...
        q = Illustration.search(
		  ...
		  { 
		    "term" => {
		      'image_content_type' => unfilter_image_type
