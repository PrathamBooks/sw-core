...
RSpec.describe DashboardController, :type => :controller do
  ...
  describe '#edit_review_form' do
    before(:each) do
      @user = FactoryGirl.create(:user)
      @story = FactoryGirl.create(:story)
      @lang = FactoryGirl.create(:english)
      @comments = {"other_comments"=>"test"}
      ReviewerComment.new(story: @story, user: @user, rating: '3', comments: @comments.to_json).save!
      @story.reindex
    end

    it 'Should return all the instance variables used in controller' do
      get :edit_review_form, {id: @story.id}
      expect(assigns(:comments)).to eq @comments
      expect(assigns(:rating)).to eq 3
    end

    it 'Should return empty hash if reviewer_comments is not correct format' do
      @story.reviewer_comment.comments = nil
      @story.reviewer_comment.save
      get :edit_review_form, {id: @story.id}
      expect(assigns(:comments)).to be_empty
    end
  end

  describe '#save_review_edit_form' do
    before(:each) do
      ...
      ReviewerComment.new(rating: '5',story: @story, user: @cm).save(validate: false)
    end

    it 'Should flag the story if there is flagging comment present' do
      sign_in @cm
      params = @params.dup
      post :save_review_edit_form, params
    ...
    it 'Should update the review comments of the story' do
      sign_in @cm
      params = @params.dup
    ...
    it "Should raise combined flash messages when the story is flagged and review comment is updated" do
      sign_in @cm
      params = @params.dup
      post :save_review_edit_form, params
      flash_reviewed_message = "Story has been reviewed successfully"
      expected_output = {"Others"=>["test"], "Story is wrongly categorised" => ["The language tag is incorrect. The story is in Bhojpuri language"]}
      story = Story.find(@story.id)
      comments = JSON.parse(story.reviewer_comment.comments)
      expect(comments["Others"]).to eq expected_output["Others"]
      expect(comments["Story is wrongly categorised"]).to eq expected_output["Story is wrongly categorised"]
      expect(flash[:notice].present?).to eq false
      flash_flag_message = "The story has been flagged. Thank you for drawing our attention to this story."
      expect(flash[:error]).to eq flash_flag_message
    end

    it 'Should failed to update the comments if the rating is not provided' do
      params = @params.dup
    ...
    it 'Should trigger email if there is a level mismatch in story in review comments' do
      sign_in @cm
      params = @params.dup
      params["reviewer_comment"]["comments"]["Story is wrongly categorised"] = [ReviewerComment::READING_LEVEL_MISMATCH_TXT]
      post :save_review_edit_form, params
      Delayed::Worker.new.work_off
      ActionMailer::Base.deliveries.last
      mail = ActionMailer::Base.deliveries.last
      expect(mail.subject).to eq 'We have a suggestion for your story'
    end
  end
