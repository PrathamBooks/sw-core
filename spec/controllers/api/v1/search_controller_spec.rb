...
#Large style illustrations generating in delayed jobs and will not available to download. 
#Please re-enable after fixing the issue. 

  context "#image_bulk_download" do
    before(:each) do
      org1 = FactoryGirl.create(:org_publisher, id: 1)
      @pub1= FactoryGirl.create(:publisher_org, :first_name => "Delhi admin", organization_id: org1.id)
      @user = FactoryGirl.create(:user, :first_name => "Test User")
      @person = FactoryGirl.create(:person, :user => @user)
      @illustration = FactoryGirl.create(:illustration, :illustrators => [@person])
      @illustration.image.reprocess_without_delay! :large
      User.reindex
      Illustration.reindex
    end
    
    it "should download the selected the images" do
      sign_in @user
      get :image_bulk_download , "JPEG" => [@illustration.id]
      expect(response.status).to eq 200
    end

    it "should download the file based on the type selected" do
      sign_in @user
      get :image_bulk_download , "JPEG" => [@illustration.id]
      file_details = DownloadHelper.create_file_with(response.body)
      get :image_bulk_download , "HiRes JPEG" => [@illustration.id]
      file_details1 = DownloadHelper.create_file_with(response.body)
      expect(file_details[:file_details].first[:size]).to be < file_details1[:file_details].first[:size]
    end

    it "should download the bulk images in zip format" do
      sign_in @user
      illustration1 = FactoryGirl.create(:illustration, :illustrators => [@person])
      illustration1.image.reprocess_without_delay! :large
      illus = [@illustration.id, illustration1.id]
      get :image_bulk_download , "JPEG" => illus
      file_details = DownloadHelper.create_file_with(response.body)
      expect(file_details[:files_present]).to eq 2
      get :image_bulk_download , "HiRes JPEG" => illus
      file_details1 = DownloadHelper.create_file_with(response.body)
      expect(response.headers["Content-Type"]).to eq("application/zip")
      expect(file_details1[:files_present]).to eq 2
      expect(file_details[:size]).to be < file_details1[:size]
    end
    
    it "should increment the download count of the respective illustration image" do
      sign_in @user
      illustration1 = FactoryGirl.create(:illustration, :illustrators => [@person])
      illustration1.image.reprocess_without_delay! :large
      download_count = IllustrationDownload.count
      get :image_bulk_download , "JPEG" => [@illustration.id]
      expect(IllustrationDownload.count).to eq download_count+1
      expect(IllustrationDownload.last.user_id).to eq @user.id
    end
  end

  context "GET Bulkdownload" do
    before(:each) do
      @user = FactoryGirl.create(:user)
      @cm_user = FactoryGirl.create(:content_manager)
      language_font_english = FactoryGirl.create(:language_font, font: 'Noto Sans', script: 'english')
      language_english = FactoryGirl.create(:english, language_font: language_font_english)
      @story1 = create_story(language: language_english)
      language_font_hindi = FactoryGirl.create(:language_font, font: 'Noto Sans Devanagari', script: 'devanagri')
      language_hindi = FactoryGirl.create(:hindi, language_font: language_font_hindi)
      @story2 = create_story(language: language_hindi)
      Story.all.each { |story| StoriesController.new.make_story_static_files(story) }
      User.reindex
      Story.reindex
    end

    it "Should downloaded stories in Bulk in a format as selected" do
      sign_in @cm_user
      stories_id = [@story1.id, @story2.id]
      params = {"PDF" => stories_id}
      get :bulk_download, params, format: :json
      response.header['Content-Type'].should == "application/zip"
      response.header['Content-Disposition'].should == "attachment; filename=\"bulk_download.zip\""
      file_details = DownloadHelper.create_file_with(response.body)
      expect(file_details[:files_present]).to eq 2
      params = {"ePub" => stories_id}
      get :bulk_download, params, format: :json
      file_details1 = DownloadHelper.create_file_with(response.body)
      expect(file_details1[:files_present]).to eq 2
      params = {"HiRes PDF" => stories_id}
      get :bulk_download, params, format: :json
      file_details2 = DownloadHelper.create_file_with(response.body)
      expect(file_details2[:files_present]).to eq 2
      expect(file_details[:size] < file_details1[:size]).to be true
      expect(file_details1[:file_details].first[:size] < file_details2[:file_details].first[:size]).to be true
    end

    it "Should increase the download count of the stories for the user" do
      sign_in @user
      stories_id = [@story1.id, @story2.id]
      params = {"PDF" => stories_id}
      get :bulk_download, params, format: :json
      expect(User.find(@user.id).story_download_count).to eq 2
    end

    it "Should download as many stories passed" do
      language_font_english = FactoryGirl.create(:language_font, font: 'Noto Sans', script: 'telugu')
      language_english = FactoryGirl.create(:telugu, language_font: language_font_english)
      13.times { story = create_story(language: language_english); StoriesController.new.make_story_static_files(story) }
      sign_in @cm_user
      ids = Story.all.map(&:id)
      params = {"PDF" => ids[2..-1] }
      get :bulk_download, params, format: :json
      file_details = DownloadHelper.create_file_with(response.body)
      expect(file_details[:files_present]).to eq ids[2..-1].count
    end
  end
end
