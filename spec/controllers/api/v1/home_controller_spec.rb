...
describe Api::V1::HomeController, :type => :controller do
  ...
  context "GET current user details" do
    it "should show current user details" do
      ...
      get :me, format: :json
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly("id", "email", "first_name", "last_name", "role", "roles", "isOrganisationMember", "hasHistory", "locale", "isLoggedIn", "bookshelf", "homePopup", "offlineBookPopupSeen", "popupsSeen", "languagePreferences", "readingLevels", "organizationRoles", "storiesDownloadLimitReached", "imagesDownloadLimitReached", "country", "name", "orgName", "hasOwnStories", "hasOwnIllustrations", "consentFlag")
  ...
  context "SHOW home page details" do
    ...
    it "should show details of editorsPick from home page" do
      ...
      expect(JSON.parse(response.body)['data']['editorsPick']['results'][0].keys).to contain_exactly("id", "title", "language", "level", "slug", "recommended", "coverImage", "authors", "contest", "editorsPick", "illustrators", "likesCount", "publisher", "readsCount", "description", "availableForOfflineMode", "storyDownloaded", "isAudio", "audioStatus", 'isGif')
    ...
    it "should show details of newArrivals from home page" do
      ...
      expect(JSON.parse(response.body)['data']['newArrivals']['results'][0].keys).to contain_exactly("id", "title", "language", "level", "slug", "recommended", "coverImage", "authors", "contest", "editorsPick", "illustrators", "likesCount", "publisher", "readsCount", "description", "availableForOfflineMode", "storyDownloaded", "isAudio", "audioStatus", 'isGif')
    ...
    it "should show details of mostRead from home page" do
      ...
      expect(JSON.parse(response.body)['data']['mostRead']['results'][0].keys).to contain_exactly("id", "title", "language", "level", "slug", "recommended", "coverImage", "authors","contest", "editorsPick", "illustrators", "likesCount", "publisher", "readsCount", "description", "availableForOfflineMode", "storyDownloaded", "isAudio", "audioStatus", 'isGif')
   ...
   # it "should show details of blogPosts from home page" do
   ...
   #    expect(x).to start_with('/public/spec/test_files/blog_posts/')
...
#Need to changed as per new recommendation algorithm
=begin
