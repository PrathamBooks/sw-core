...
#SHOW
  context "SHOW/GET Illustrations Details" do
    it "should show the illustration details" do
      ...
      expect_status(200)
      expect(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly('title', 'id', 'slug', 'categories', 'styles', 'license_type', 'attribution_text', 'liked', 'likesCount', 'readsCount', 'isPulledDown', 'isFlagged', 'tags', 'imageUrls', 'illustrators', 'similarillustrations', 'downloadLinks', 'publisher', 'usedIn', 'downloadLimitReached', 'illustrationAccess', "dateModified", "datePublished")
      ...
      expect_json('data.categories', [{:name=>nil, :query=>'Category1'}])
      expect_json('data.styles', [{:name=>"Style1", :query=>"Style1"}])
