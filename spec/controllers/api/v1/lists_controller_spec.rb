...
describe Api::V1::ListsController, :type => :controller do
  ...
  context "POST/Create List" do
    ...
    it "should create a list with title" do
      ...
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly("title","id","count","liked","likeCount", "readCount","canDelete","author","description","slug","books", "categories", "dateModified", "datePublished")
      ...
      expect(x).to end_with("/assets/profile_image.jpg")
  ...
  context "SHOW/GET List" do
    it "should show the list" do
      ...
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly("title","id","count","liked", "likeCount", "readCount","canDelete","author","description","slug","books", "organisation", "categories", "dateModified", "datePublished")
      expect_json(ok: true)
      categories_array = []
  ...
  context "CREATE List and ADD a Story" do
    it "should add a story to the list" do
      ...
      expect(JSON.parse(response.body)['data']['books'][0].keys).to contain_exactly("title","id","language","level","slug","synopsis","usageInstructions","coverImage","authors","isAudio")
  ...
  context "Filters" do
    it "should filter the list with given parameters" do
      ...
      list1 = FactoryGirl.create(:list, user: @user, :title => "List1", :categories => [fiction])
      list2 = FactoryGirl.create(:list, user: @user, :title => "List2", :categories => [funny])
      List.reindex
      Story.reindex
      ...
      expect_json('data.filters.0.queryValues.1.name', 'Translated Fiction')
      expect_json('data.filters.0.queryValues.1.queryValue', 'Fiction')
      expect_json('data.filters.0.queryValues.2.name', 'Translated Funny')
      expect_json('data.filters.0.queryValues.2.queryValue', 'Funny')
