    ...
    describe "search" do
        ...
        it "should search by name" do
         ...
         it "should filter by categories" do
          ...
          search_params = {query: "Name", categories: [category1.name, ''], styles: [],license_types: [], image_mode: 'false', is_publisher_cm: 'false', content_manager: 'false', bulk_download: nil, download_format: nil}
          get :index, search: search_params

          expect(assigns(:results).collect(&:name).include?(i1.name)).to be true
          expect(assigns(:results).collect(&:name).include?(i2.name)).to be false
          expect(assigns(:search_params)).to eql({'query'=>'Name', 'categories'=>[category1.name], 'styles' => [],'license_types' => [], 'illustrators' => nil, 'organization' => nil, "image_mode" => "false", "editor_fav_images"=>nil, "favorites_of_story"=>nil, "illustrator_slugs" => nil, "is_organization_cm"=>false, "content_manager"=>nil, "tags" => nil, "download_format" => nil, "bulk_download" => nil, "allow_gif_images" => nil, "image_content_type" => nil})
          expect(assigns(:filters)).to eql({categories: [category1.name], image_mode: "false", is_pulled_down: false, image_content_type: {:not=>"image/gif"}})
        end
         ...
         it "should filter by styles" do
          ...
          search_params = {query: "Name", categories: [], styles: [style1.name,''],license_types: [],image_mode: 'false', is_organization_cm: 'false', content_manager: 'false', bulk_download: nil, download_format: nil}
          get :index, search: search_params

          expect(assigns(:results).collect(&:name).include?(i1.name)).to be true
          expect(assigns(:results).collect(&:name).include?(i2.name)).to be false
          expect(assigns(:search_params)).to eql({'query'=>'Name', 'categories'=>[], 'styles' => [style1.name],'license_types' => [], 'illustrators' => nil, 'organization' => nil, 'image_mode' => "false", "editor_fav_images"=>nil, "favorites_of_story"=>nil, "illustrator_slugs" => nil, "is_organization_cm"=>false, "content_manager"=>nil, "tags" => nil, "download_format" => nil, "bulk_download" => nil, "allow_gif_images" => nil, "image_content_type" => nil})
          expect(assigns(:filters)).to eql({styles: [style1.name],image_mode: "false", is_pulled_down: false, image_content_type: {:not=>"image/gif"}})
         end
  ...
  describe "POST create" do
    ...
    describe "when uploader is a publisher" do
      ...
      it "should create an illustrator account if not present" do
        ...
        attributes.merge!(illustrators_attributes: {0 => {:email => illustrators_email, :first_name => illustrator_first_name, :last_name => illustrator_last_name, :consent_flag => true }})
      ...
      it "should set the illustrator's bio" do
        ...
        attributes.merge!(illustrators_attributes: {0 => {:email => illustrators_email, :first_name => illustrator_first_name, :last_name => illustrator_last_name , :bio => illustrator_bio, :consent_flag => true}})
        post :create, illustration: attributes
