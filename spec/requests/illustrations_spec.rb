...
RSpec.describe "Api::V1::Illustrations::Requests", :type => :request do
  ...
  #   end
    it "should show the Illustration details" do
      ...
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly('title', 'id', 'slug', 'categories', 'styles', 'license_type', 'attribution_text', 'liked', 'likesCount', 'readsCount', 'isPulledDown', 'tags', 'imageUrls', 'illustrators', 'similarillustrations', 'downloadLinks', 'isFlagged', 'publisher', 'usedIn', 'downloadLimitReached', 'illustrationAccess', "dateModified", "datePublished")
