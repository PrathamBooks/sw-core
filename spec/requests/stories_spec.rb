...
#SHOW
  context "GET Story Details" do
    ...
    it "should show the story details" do
      ...
      similar_story1 = FactoryGirl.create(:level_1_story,language: english, :tag_list => ["New tag"], :title => "Original Story", :authors => [user1], :status => Story.statuses[:published], :donor_id => donor.id)
      ...
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly('name', 'id', 'language', 'level', 'slug', 'status', 'description', 'isTranslation', 'readsCount', 'likesCount', 'readingListMembershipCount', 'copyrightNotice', 'orientation', 'recommended', 'canEdit', 'publisher', 'publishedTime', 'donor', 'contest', 'similarBooks', 'translations', 'coverImage', 'tags', 'downloadLinks', 'authors', 'originalStory', 'illustrators', 'lists', "availableForOfflineMode", "editorsPick", "externalLink", "googleFormDownloadLink", "isFlagged", "languageCount", "liked", "versionCount", "isRelevelled", "photographers", "downloadLimitReached", "categories", "publishedDate", "isAudio", "lastPagePosition", 'isGif')
...
#Filters
  context "GET Story details after filter" do
    it "should show the story details with filter" do
      ...
      expect(JSON.parse(response.body).keys).to contain_exactly('ok', 'data', 'sortOptions', 'downloadType')
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly('language', 'publisher', 'category', 'level', "derivation_type", "status", 'story_type')
