...
describe Illustration, :type => :model do
  ...
  it "should validate that it is a valid image content type for normal user" do
    p = FactoryGirl.create(:person)
    user = FactoryGirl.create(:user)
    i = FactoryGirl.create(:illustration, illustrators: [p], uploader: user)
    i.valid?
    i.errors[:base].should_not include("PNG and JPEG are the only supported formats")
    ...
    i.valid?
    i.errors[:base].should_not include("PNG and JPEG are the only supported formats")
    i.image_content_type = "text/pain"
    i.valid?
    i.errors[:base].should include("PNG and JPEG are the only supported formats")
    i.image_content_type = "image/gif"
    i.valid?
    i.errors[:base].should include("PNG and JPEG are the only supported formats")
  end

  it "should validate that it is a valid image content type for content manager" do
    p = FactoryGirl.create(:person)
    user = FactoryGirl.create(:user, site_roles: "content_manager")
    i = FactoryGirl.create(:illustration, illustrators: [p], uploader: user)
    i.valid?
    i.errors[:base].should_not include("GIF, PNG and JPEG are the only supported formats")
    i.image_content_type = "image/png"
    i.valid?
    i.errors[:base].should_not include("GIF, PNG and JPEG are the only supported formats")
    i.image_content_type = "image/jpeg"
    ...
    i.image_content_type = "text/pain"
    i.valid?
    i.errors[:base].should include("GIF, PNG and JPEG are the only supported formats")
  end
