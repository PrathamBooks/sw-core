...
When (/^(?:|I )click link "([^"]*)"$/) do |link|
  first(:link, link).trigger('click') rescue click_link link
end
...
Then (/^(?:|I )expect "([^"]*)"$/) do |text|
  ...
  if page.respond_to? :should
    expect(page.has_text? text).to be true
  else
    assert page.has_content?(text)
  end
end
...
And(/^I signout from storyweaver$/)do
  @sw.user_dash_board.select_dashboard("Sign Out")
end
