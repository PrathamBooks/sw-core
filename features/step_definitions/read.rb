...
When(/^I create "([^"]*)" stories with category "([^"]*)" and language as "([^"]*)" with reading_level "([^"]*)" from the author as "([^"]*)" with the title "([^"]*)" and the flaggings_count as "([^"]*)" and with org "([^"]*)"$/) do |number_of_stories, category, language, reading_level, author, title, flaggings_count, organization|
 language_record = Language.find_by_name(language)
 author = User.find_by_email(author)
 category_record = StoryCategory.find_by_name(category)
 organization = Organization.find_by_email(organization)
...
When (/^I create list with "([^"]*)" stories by "([^"]*)"$/) do |story_count, title|
  create_list({story_count: "#{story_count}".to_i, list_title: title})
end

When (/^I create list with "([^"]*)" stories with category "([^"]*)" has number of reads as "([^"]*)" with likes of "([^"]*)" by "([^"]*)"$/) do |story_count, category, list_reads, list_likes, title|
  category_record = ListCategory.find_by_name(category)
  create_list({story_count: "#{story_count}".to_i, category: category_record, reads: list_reads, likes: list_likes, list_title: title})
end